...
import { usePinGridVisualConfigContext } from "@/context/visualConfig/pinGrid";

const PinGridVisual = ({
  coordinateMapper,
  ...props
}: GroupProps & VisualProps) => {
  const { nRows, nCols, unitSideLength } = usePinGridVisualConfigContext();
  ...
  const radius = Math.max(nCols, nRows) * 1.1 * unitSideLength;
